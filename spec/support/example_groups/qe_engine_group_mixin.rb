module QEEngineGroupMixin
  ...
  def create_scheduler_session(opts = {})
      ...
      # Create session with default session factory parameters injected
      session_params = { customer_id: @customer.id }
      session_params[:business_id] = opts[:business].id if opts[:business]
      @session  = create(:scheduler_session, session_params)
